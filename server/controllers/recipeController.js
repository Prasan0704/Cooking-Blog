...
exports.exploreRecipie = async(req, res) => {
    try {
        ...
        if (req.query.num) {
            ...
            if (num > 0) {
                recipie.ingredients.forEach((ingredient, index) => {
                    const regex = /^(\d+)(\s*)(.*)$/; // regular expression to extract the quantity, unit and ingredient name
                    const matches = ingredient.match(regex);
                    if (matches) {
                        const quantity = parseInt(matches[1]) * num; // extract the quantity and multiply by num
                        const unit = matches[2];
                        const name = matches[3];
                        recipie.ingredients[index] = `${quantity}${unit}${name}`; // update the ingredient string with the new quantity
                    }
                });
            }
        }

        res.render('recipie', { title: 'Cooking Blog - Recipe', recipie });
    } catch (error) {
        res.status(500).send({ message: error.message || "Error Occured" });
    }
}
...
exports.exploreRecipieWithMorePerson = async(req, res) => {
    try {
        let recipeId = req.params.id;
        const recipie = await Recipe.findById(recipeId);
        res.render('recipie', { title: 'Cooking Blog - Recipie',  recipie });
...
exports.searchRecipie = async(req,res) => {
    try{
        ...
        let recipie = await Recipe.find({$text: {$search: searchTerm, $diacriticSensitive: true}});
...
exports.exploreLatest = async(req,res) => {
    try{
        ...
        const recipie = await Recipe.find({}).sort({_id: -1 }).limit(limitNumber);
        res.render('explore-latest', { title: 'Cooking Blog - Categories',  recipie });
    } catch(error){
        res.status(500).send({message: error.message || "Error Occured" });
    }
}
...
exports.exploreRandom = async(req,res) => {
    try{
        ...
        let recipie = await Recipe.findOne().skip(random).exec();
        res.render('random-recipie', { title: 'Cooking Blog - Categories',  recipie });
    } catch(error){
        res.status(500).send({message: error.message || "Error Occured" });
    }
}
...
exports.SubmitRecipie = async(req,res) => {
    ...
    res.render('submit-recipie', { title: 'Cooking Blog - Submit Recipie', infoSubmitObj, infoErrorObj});
}
...
/**
 * GET /Explore Qunatity 
 * Explore Qunatity
 */

exports.exploreQunatity = async(req,res) => {
    res.render('Quantity');
}
...
//             "description": `Wash the mussels thoroughly, discarding any that aren’t tightly closed. Trim and finely slice the spring onions, peel and finely slice the garlic. Pick and set aside the coriander leaves, then finely chop the stalks. Cut the lemongrass into 4 pieces, then finely slice the chilli. In a wide saucepan, heat a little groundnut oil and soften the spring onion, garlic, coriander stalks, lemongrass and most of the red chilli for around 5 minutes.
